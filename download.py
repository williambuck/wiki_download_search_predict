    ...
    def get_all_page_contents_from_category(self, category, overwrite=False, second_run=False, nesting_level=0):
        ...
        if overwrite:
            ...
            if Q == 'y':
                ...
                print('Dropped and proceeding with download.')
            else:
        ...
        if nesting_level != 0:
            ...
            print('There were no subcategory pages included for "{}".'.format(category))
            return
...
# This is the user interface for the downloader that starts when the file is run
db_name = input('Database name: ')
db_name = re.sub('\s', '_', db_name)

cat_search = input('Category to download: ')
nest_lvl = int(input('Nesting level: '))
overwrite = input('Overwrite? [y/n] ')

print(f'\nStarting download of category "{cat_search}"')

db_mongo = store_wiki_contents_in_mongo(db_name)

if overwrite=='y' or overwrite=='yes':
    db_mongo.get_all_page_contents_from_category(cat_search, overwrite=True, nesting_level=nest_lvl)
else:
    db_mongo.get_all_page_contents_from_category(cat_search, nesting_level=nest_lvl)


continue_downloading = input('\nContinue downloading? [y/n] ')
while continue_downloading == 'y' or continue_downloading == 'yes':
...
merge_df = input('Merge all downloads to DataFrame and pickle? [y/n] ')
if merge_df == 'y' or  merge_df == 'yes':
    dmmamc = df_maker_merge_all_mongo_content()
    final_df = dmmamc.merge_collections(db_name)
    pd.to_pickle(final_df, 'Data/{}_download_merged_df.p'.format(db_name))
    print('Completed merge.')
else:
    print('Finished without merging. Download information still availble in MongoDB "client.{}".'.format(db_name))
